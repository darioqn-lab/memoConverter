...
import os, csv, json, tempfile, configparser, sys, datetime
            ...
            if ext == ".pdf":
                ...
                try:
                    ...
                    result = subprocess.run(
                        ["antiword", tmp.name], capture_output=True, timeout=10
                    )
                    texto = result.stdout.decode("utf-8", errors="replace")
                    if not texto.strip():
                ...
                from memorial_parser import ler_xls
                texto, err = ler_xls(tmp.name)
                if err and not texto:
                    return jsonify({"erro": err}), 500

            elif ext == ".txt":
                from memorial_parser import ler_txt
                texto, err = ler_txt(tmp.name)
                if err and not texto:
                    return jsonify({"erro": err}), 500
                ...
                return jsonify({"erro": "Formato nao suportado. Use PDF, Word (.docx/.doc), Excel (.xls/.xlsx), Texto (.txt) ou imagem (JPG, PNG)."}), 400
