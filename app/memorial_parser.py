...
def ler_txt(path):
    """Extrai texto de arquivo .txt. Retorna (texto, erro_ou_None)."""
    for enc in ("utf-8-sig", "utf-8", "latin-1", "cp1252"):
        try:
            ...
            continue
    return "", "Nao foi possivel ler o arquivo de texto."


def ler_xls(path):
    """Extrai texto de planilha .xls/.xlsx. Retorna (texto, erro_ou_None)."""
    try:
        import openpyxl
    ...
    try:
        ...
        for ws in wb.sheets():
            for i in range(ws.nrows):
                celulas = [str(ws.cell_value(i, j)).strip()
        ...
        return "", "openpyxl nao instalado. Execute: pip install openpyxl"
    except Exception as e:
        return "", str(e)
...
def extrair_vertices(texto):
    ...
    2. Tabela com azimute/distância (formato memorial brasileiro)
    3. Par inline E,N separados por vírgula (formato CSV)
    4. Par inline E N separados por espaço
    5. Tabela em blocos
    6. Coordenadas geograficas (DMS) — fallback
    """
    ...
    # tabela OCR em blocos (cada célula em linha separada)
    v_ocr = _extrair_ocr_blocos(texto)
    if v_ocr:
        return v_ocr

    # tabela com azimute/distância (Ponto | Azimute | Distância | E | N)
    v_tabela_az = _extrair_tabela_azimute(texto)
    if v_tabela_az:
        return v_tabela_az

    # par inline (E,N separados por vírgula ou espaço)
...
def _extrair_ocr_blocos(texto):
    """
    Parser para tabelas OCR onde cada célula ocupa uma linha separada.
    Padrão detectado:
      Ponto (ex: P1-P2)
      Azimute (ex: 238°39'44)
      Distância (ex: 13,04)
      E (ex: 525.262,745)
      [N(m)]  ← cabeçalho que pode aparecer no meio
      N (ex: 8.549.485,042)
    """
    import re

    _UTM_RE = re.compile(r'^\d{3}(?:[.,]\d{3})*(?:[.,]\d{1,4})?$')
    _N_RE   = re.compile(r'^\d{7}(?:[.,]\d{3})*(?:[.,]\d{1,4})?$')
    _PONTO  = re.compile(r'^[A-Za-z]{1,3}[-_]?\d{1,3}(?:[-–][A-Za-z]{0,3}\d{0,3})?$')
    _SKIP   = re.compile(r'^(Ponto|Azimute|Dist[âa]ncia.*|E\(m\)|N\(m\)|Tabela.*|A\d.*)$', re.IGNORECASE)
    ...
    linhas = [l for l in linhas if not _SKIP.match(l)]
    ...
    while i < len(linhas):
        l = linhas[i]
        # Detecta início de registro: identificador de ponto
        if _PONTO.match(l):
            nome = re.split(r'[-–]', l)[0].strip()  # pega só "P1" de "P1-P2"
            # Avança pulando azimute e distância
            j = i + 1
            e_val = None
            n_val = None
            while j < len(linhas) and j < i + 6:
                cand = linhas[j]
                # Pula azimute (tem °) e distância pequena
                if '°' in cand or "'" in cand:
                    j += 1; continue
                # Pula cabeçalhos que escaparam
                if _SKIP.match(cand):
                    j += 1; continue
                try:
                    v = parse_num(cand)
                    if 100000 <= v <= 9999999:
                        if e_val is None:
                            e_val = v
                        elif n_val is None and v > 1000000:
                            n_val = v
                            break
                except Exception:
                    pass
            ...
            if e_val and n_val:
                from memorial_parser import _classificar_en, _is_utm
                if _is_utm(e_val) and _is_utm(n_val):
                    e, n = _classificar_en(e_val, n_val)
...
def _extrair_tabela_azimute(texto):
    """
    Extrai coordenadas de tabelas no formato:
    Ponto | Azimute | Distância(m) | E(m) | N(m)
    Ex:
      P1-P2  238°39'44"  13,04  525.262,745  8.549.485,042
    """
    # Padrão: identificador, azimute (opcional), distância (opcional), E, N
    pat = re.compile(
        r"([A-Za-z]{1,3}[-_]?\d{1,3}(?:[-–][A-Za-z]{0,3}\d{0,3})?)"  # Ponto ex: P1-P2
        r"[^\n]{0,80}?"                                                   # Azimute + distância
        r"(\d{3}(?:[.,]\d{3})*(?:[.,]\d{1,4})?)"                        # E
        r"\s+"
        r"(\d{7}(?:[.,]\d{3})*(?:[.,]\d{1,4})?)",                       # N (7+ dígitos)
        re.MULTILINE
    )
    ...
    for m in pat.finditer(texto):
        try:
            ...
            # Extrai só a parte "De" (antes do hífen) se for "P1-P2"
            nome_de = re.split(r"[-–]", nome)[0].strip()

            e_str = m.group(2).replace(".", "").replace(",", ".")
            n_str = m.group(3).replace(".", "").replace(",", ".")
            e, n = float(e_str), float(n_str)

            if not (_is_utm(e) and _is_utm(n)):
                continue
            e, n = _classificar_en(e, n)
            chave = (round(e, 1), round(n, 1))
            if chave in seen:
                continue
            seen.add(chave)
...
def _extrair_pares_inline(texto):
    ...
    # Par de numeros UTM — separados por espaço, vírgula, ponto-e-vírgula ou tabulação
    pat = re.compile(
        r"(\d{5,7}[.,]\d{2,6})\s*[,;\t]\s*(\d{7,8}[.,]\d{2,6})"
        r"|"
