...
def usar_gemini():
    ...
    if not api_key:
        return jsonify({"erro": "Chave do Gemini nao configurada."}), 400
...
def exportar():
    ...
    # Timestamp compatível com todas as versões do QGIS 3.18+
    ts = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    nome_arquivo = f"vertices_memorial_{ts}.csv"
    ...
    # Forma mais estável de enviar o arquivo (funciona no QGIS 3.18)
    return send_file(
        tmp.name,
        mimetype="text/csv",
        as_attachment=True,
...
if __name__ == "__main__":
    ...
    print(f"\n  Memorial Converter v{APP_VERSION} — http://localhost:{port}\n")
